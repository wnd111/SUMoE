...
from baselines.src.utils.config import handle_config, save_args, handle_args_to_ignore
from datasets import load_dataset
from baselines.src.metrics import load_metric
from baselines.src.utils.reproducibility import save_git_info
from baselines.src.utils.duplicates import drop_duplicates_in_input
from baselines.src.models.hierarchical_attention_model import HierarchicalAttentionModel, HierarchicalAttentionConfig
from baselines.src.models.sparse_moe_model import SparseMoEModel, SparseMoEConfig
...
def main():
    ...
    if data_args.dataset_name is not None:
        try:
            ...
            logger.warning(f"Failed to download dataset from Hugging Face: {e}")
            logger.info("Attempting to load from local directory...")
            # 如果下载失败，尝试从本地加载
            if data_args.data_dir is not None and os.path.exists(data_args.data_dir):
                seq2seq_dataset = load_dataset(
                    "json",
                    data_files={
                        "train": os.path.join(data_args.data_dir, "train.jsonl"),
                        "validation": os.path.join(data_args.data_dir, "validation.jsonl"),
                        "test": os.path.join(data_args.data_dir, "test.jsonl"),
                    },
                    cache_dir=model_args.cache_dir,
                )
            else:
                raise ValueError(f"Dataset not found locally at {data_args.data_dir} and failed to download from Hugging Face")
    else:
        # 从文件加载
        data_files = {}
        ...
        seq2seq_dataset = load_dataset(extension, data_files=data_files, cache_dir=model_args.cache_dir)
    ...
    # 修改配置以支持稀疏MoE
    config.use_sparse_moe = True
    config.num_experts = 8
    config.expert_capacity = 32
    config.router_jitter_noise = 0.1
    ...
    if training_args.do_eval:
        ...
        logger.info(f"Starting evaluation on {len(eval_dataset)} examples")
